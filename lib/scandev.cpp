    ...
    if (_scandev_first)
    {
        for (SCANDEV_T *device = _scandev_first; device; device = device->next)
        {
            ...
            */
            (*callback)("<tr>"
                        "<td>" +
                        String(device->present ? "✅" : "❌") + "</td>"
                                                              "<td>" +
                        String(device->addr.toString().c_str()) + "</td>"
                                                                  "<td>" +
                        String((device->name[0]) ? device->name : "-") + "</td>"
                                                                         "<td>" +
                        String((device->manufacturer_id != BLE_MANUFACTURER_ID_UNKNOWN) ? BLEManufacturerIdHex(device->manufacturer_id) : "-") + "</td>"
                                                                                                                                                 "<td>" +
                        String((device->manufacturer_id != BLE_MANUFACTURER_ID_UNKNOWN) ? device->manufacturer : "-") + "</td>"
                                                                                                                        "<td>" +
                        String(device->rssi) + "</td>"
                                               "<td>" +
                        String(RSSI2METER(device->rssi)) + "</td>"
                                                           "<td>" +
                        String(TimeToString(device->last_seen)) + "</td>"
                                                                  "<td>" +
                        ((device->has_battery) ? String(device->battery_level) : String("-")) + "</td>"
                                                                                                "</tr>");
        }
    }
    else
